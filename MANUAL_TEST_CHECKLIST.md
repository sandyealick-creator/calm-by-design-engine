...
- [ ] 🔲 Needs Cloud Run. For a `--no-traffic` candidate, require `latestCreatedRevisionName` to equal the candidate, its own revision evidence to report exactly one `Ready=True`, and `latestReadyRevisionName` to remain the fixed traffic-serving baseline. This proves creation, readiness, and production exposure separately.
